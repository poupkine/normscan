...
export const ActionCard: FC = () => {
  ...
  // === НОВАЯ ФУНКЦИЯ: СКАЧИВАНИЕ ОТЧЁТА ===
  const downloadReport = async () => {
    try {
      const response = await fetch('/api/download_report');
      if (!response.ok) {
        throw new Error('Отчёт не найден на сервере');
      }
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'normscan_report.xlsx';
  ...
  useEffect(() => {
    if (fileStatus === 'success' && fileData) {
      ...
      // downloadReport(); // <-- вызываем скачивание
    ...
    if (fileListStatus === 'success' && fileListData) {
      ...
      // ✅ Скачиваем ТОЛЬКО если отчёт точно создан
    if (fileListData.report_available) {
      setTimeout(downloadReport, 500); // небольшая пауза для надёжности
    } else {
      alert('⚠️ Отчёт не был сгенерирован. Попробуйте позже.');
    }
